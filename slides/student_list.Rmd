...
# slidify('slides/student_list.Rmd')
  # delete "cache" in order for figures to display properly
    # C:\Users\ozanj\Documents\student_list_hsls\slides\.cache
...
A two-sided matching problem in which market allocates students to colleges `r citep(bib[c('RN2133','RN2247')])`
...
- Recruiting visits by public research univs   `r cite(bib[c('RN4758','RN4759')])`
...
```{r orders-filters-combined, fig.height = 5}
orders_filters %>% 
  filter(is_asu == 'all') %>% 
  group_by(filters) %>% 
  summarise(
...
<br>
also see: `r citet(bib['RN4894'])` for effect of university purchasing profile on college choice
...
Selection devices allocate individuals to categories based on input factors `r citep(bib[c('RN4778')])`
...
Standardized selection devices and racial inequality
  ...
  - Do not reduce inequality due to structural racism `r citep(bib[c('RN4814')])`
    - Structural racism is “systematic racial bias embedded in the ‘normal’ functions of laws and social relations,” whereby processes viewed as neutral or common-sense systematically disadvantage marginalized groups `r cite(bib['RN4760'], eg = F, suffix = 'p. 1143')`
...
- Zip code an input in algorithm that predicts crime recidivism `r citep(bib[c(c('RN4849','RN4775'))])`
...
- Geodemographic classifications classify localities by consumer behavior `r citep(bib[c('RN4880')])`
  - CB Geodemographic filters classify census tracts by past college enrollment `r citep(bib[c('RN4565')])`
...
- Racial/ethnic differences in test-taking and test scores a function of historic/contemporary school segregation, differences in school funding and access to curricula `r cite(bib[c(c('RN4893','RN4853'))])`
<!-- - Concerns about bias in question items `r cite(bib[c(c('RN6012', 'RN6011'))])` -->
...
- background section; 
  - [DONE] replace current enrollment funnel w/ the one from here (might ask crystal to do this): https://ticas.org/wp-content/uploads/2022/09/Geodemographics-of-Student-List-Purchases_A-First-Look.pdf 
...
- Parallel origins of consumer credit reports and student lists `r citep(bib[c('RN2247','RN4880')])`
