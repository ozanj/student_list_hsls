...
We briefly highlight key findings that emerge sociological scholarship on product utilization. First, scholarship tends to find that administrative discretion over selection devices causes structural inequality to increase, but particular product features or conditions may reduce or amplify inequality [CITE] [@RN4883; @RN4881; @RN4795]. Second, compared to standardized selection devices, discretionary selection devices allow the explicit or implicit racial bias of individual decision-makers to affect selection decisions [@RN4794; @RN4801]. Third, Americans dramatically underestimate the magnitude of racial income inequality [@RN4884]. @RN4885[p. 142] describes this "racial ignorance" as a means of "white knowledge evasion" that "reinforces white domination." Discretionary selection devices that incorporate racialized inputs are likely to amplify racial inequality because decision-makers may be ignorant about how selection inputs interact with on-the-ground patterns of racial inequality [@RN4801] and/or because decision-makers may view racialized inputs objective, colorblind measures of merit [@RN4882; @RN4866]. Finally transparency and accountability are necessary conditions to prevent ascriptive bias in discretionary selection processes [@RN4881; @RN4883]. Here, transparency means that decisions about selection criteria are clear to all stakeholders and accountability means decision-makers face consequences for utilizing biased selection criteria.
<!--
...
Drawing from the utilization literature, we raise three ideas about the relationship between administrative discretion and racial inequality in student list purchases. First, universities likely select academic achievement filters that are consistent with admissions standards. While race-based differences in standardized testing may be driven by structural inequality, most admissions offices viewed test scores objective measures of achievement or aptitude [@RN3699], until quite recently. Therefore, we expect selective institutions are more likely to filter on standardized test scores compared to less selective institutions and are likely to filter on higher score thresholds. [ADD SENTENCE?: GIVEN LIMITED SAMPLE SIZE OF 14 PUBLIC UNIVS, WE USE CARNEGIE CATEGORY OF RESEARCH VS. MA/DOCTORAL AS A CRUDE PROXY FOR SELECTIVTY]

Second, we expect that filtering on multiple structurally racist inputs has a compounding effect on racial inequality in which prospects are included versus excluded. Filtering on multiple search criteria facilitates micro-targeting of desired prospects. Micro-targeting has become a branding strategy for student list products. Indeed, College Board Student Search promises to “create a real pipeline of best-fit prospects” [@RN1623]. Thus, student list products are complicated, powerful products for precisely identifying prospects. However, student list products generally do not highlight how the characteristics of targeted prospects compare to the demographics of their surrounding [CITE]. The flip-side of micro-targeting is exclusion. Considering the extent to which Americans underestimate race-based income inequality [CITE], we expect that student list purchases that filter on multiple racialized inputs yield dramatic racial inequality because these multiple filters interact with local patterns of racial segregation in ways that are difficult to predict. Additionally, we expect that using student list filters that utilize predictive analytics (e.g., geomarket, geodemographic segment) is likely to yield dramatic racial inequality because these filters categorize schools and communities based on historic patterns of college enrollment.

Third, some colleges may utilize discretion offered by student list products to explicitly target underrepresented populations. Indeed, College Board and ACT products are designed to facilitate this goal because colleges may filter by race, ethnicity, gender, first-generation status, etc. Colleges may purchase lists for certain racial/ethnic groups using one set of score thresholds and purchase names for other racial/ethnic groups using a different set of score thresholds. Purchases that explicitly target underrepresented populations may be more or less inclusive of students from communities of color. Given the complexity of student list products and incomplete knowledge about race-based income inequaltiy, we are concerned that student list purchases designed to overcome one inequality in college access may unintentionally create other social inequalities. For example, purchases designed to target "women in STEM" may yield racial or socioeconomic inequality while purchases that explicitly target underrepresented racial minorities may largely exclude communities of color.
