...
**Author response**. The contribution this manuscript makes differs from a purely empirical contribution. Consistent with EEPA "Critical Approaches to Education Policy Research" call for proposals, the primary contributions of this research are: (contribution 1) motivate future critical education policy research on third-party products and vendors (which have not been the object of inquiry in prior research); and (contribution 2) to introduce concepts from sociology that can be used to study these topics with a focus on racial inequality. Reviewers 1 and 2 helpfully explained why our original conceptual framework fell short and we believe the revised framework (discussed below) is much stronger.

We now move to the empirics. Given that racial inequality in student list products has never been studied before, the descriptive analyses of individual search filters are important for demonstrating basic relationships between search filters and racial composition. Although the expected findings can be inferred from prior literature, it is more powerful to demonstrate these findings empirically. Additionally, like Norris's [-@RN4786] analysis of Moody's city credit rating algorithm, our simple analyses of HSLS shows future education researchers that third-party products that have not been studied before can be studied by applying simple descriptive statistics to publicly available data. This is part of Contribution 1.
...
**Author response**. We agree with this suggestion. In the revised manuscript, the first paragraph focuses on racial inequality in college access and provides statistics based on [KARINA]. The introduction then shows that the emergence of student lists was fundamentally based on the standardized college entrance exam, which sociologists argue are mechanisms that amplify historic structural inequality. We agree with Reviewer 3 that framing the manuscript vis-a-vis racial inequality in college access helpls situate the so-what of studying student lists. However, we do not go in greater depth about racial inequality in college access because the manuscript is very long and we make the assumption that the assumption that most readers know that racial inequality in college access exists. Additionally, we begin the discussion section re-stating the problem of racial inequality in college access. 
...
**Author response**. @RN4778 are IMPORTANT BUT NOT TEXT TEXT TEXT
...
**Author response**. KARINA -- CAN YOU HANDLE THIS ONE

1. Unclear organization of the conceptual framework
...
1. Improve organization of discussion section and reduce redundancy
