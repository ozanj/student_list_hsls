...
- Paragraph 2 states that selection devices are important objects of critical policy research. The focus on selection devices is consistent with the long research tradition on stratification/tracking and many contemporary third-party products can be usefully conceptualized as selection devices. We state that the utility of our conceptual framework is in casting attention to the inputs used by selection devices, which motivates future scholarship to deconstruct selection devices.
...
X. Tighten and clarify conceptual framework (particularly around discretionary versus standardized selection devices)

> **Editor concern**. *I also believe the conceptual framework section can be tightened a bit. For example,while I agree with the reviewers about the confusion with “standardized selection devices,” the paper neverreally engages with these points in the analysis nor does it really return to this point in the discussionsection. I wonder whether the discussion of discretionary versus standardized actually needs to be includedin the paper. By removing that, I think the front part of the paper will focus more strongly on the way theinputs are racialized (and would also provide more space to address the point I mention below). The firsttwo paragraphs of the results could also likely be trimmed. I’m not wedded to any one suggestion, just thatthis paper get much closer to the page requirements for EEPA which can be fairly easily done by selectivelyshifting figures that are adequately described in the text and tightening language a bit (and if the selectiondevices piece remains, the paper needs to address the concerns brought up by the reviewers).*
