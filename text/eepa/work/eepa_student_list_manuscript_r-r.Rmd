...
## Selection Devices
 ...
 <!-- Many literatures in sociology are concerned with selection processes, which can be defined as processes for "making categorical decisions about individuals" [@RN4778, p. 349].-->

IDEAS THAT NEED TO BE WORKED IN HERE

- CALL IT "SELECTION ROUTINES" RATHER THAN "SELECTION PROCESSES"
- DEVELOP A MEANINGFUL DEFINITION OF ALGORITHMIC PRODUCTS
  - SOMETHING TO DO WITH SOFTWARE?
- DOMAINS WE FOCUS ON
  - ADMINISTRATIVE DISCRETION VS. STANDARDIZATION
  - KINDS OF INPUTS THAT DETERMINE OUTCOMES
    - USE HISTORICAL BEHAVIOR OF SIMILAR/NEARBY PEOPLE AS AN INPUT?

Many literatures in sociology are concerned with selection processes [?ROUTINES?], which allocate individuals to categories [BASED ON SOME SET OF INPUT FACTORS]. Examples of selection processes include college admissions, applications for credit, prison sentencing, child welfare decisions []. One dimension by which selection selection decisions depend on individual administrative discretion versus standardized, rule-based criteria. Discretionary selection processes rely on the judgment of individual evaluators who -- in professional domains -- exercise judgment about individual cases based on professional norms about which inputs matter and how to evaluate them. For example, holistic admissions can be considered a discretionary selection process where admissions readers [SOMETHING LIKE THEY EXERCISE JUDGMENT BUT ARE TRAINED ABOUT WHAT DEFINES MERIT].

By contrast, @RN4778 define standardized selection devices as formulaic "standardization tools designed for making categorical decisions about individuals" [RN4778]. 

One dimension by which standardized selection devices differ is the process for determining the weight attached to each input. Actuarial selection devices, pioneered by the insurance industry, proceed in two steps. First, apply statistical techniques (e.g., regression) to previous cases in order to identify the determinants of an outcome. Second, apply these results (e.g., regression coefficients) to future cases in order to make predictions and to assign levels of risk to each case. In child welfare, for example, the "Structured Decision Making Model" (SDM) is an actuarial selection device that yields recommendations about whether children should be placed in protective care based on a standardized survey instrument designed to discern the likelihood of future abuse or neglect [CITE]. In nonactuarial standardized selection devices, the formula which assigns weights to each input is not determined by statistical analyses of past cases. Prior to 2003, the University of Michigan evaluated applications by assigning points to characteristics (e.g., GPA, ACT/SAT score, alumni, underrepresented racial/ethnic minority) and admitting applicants above 100 points. However, decisions about the number of points to assign each characteristics were based on considerations of organizational mission -- including the goal of reducing racial inequality -- rather than regression coefficients from past analyses of student success.

Scholarship from sociology has examined the relationship between selection devices and racial inequality. @RN4778 state that standardized selection devices can promote racial inequity _if_ the primary source of inequality is explicit or implicit racial bias from individual decision-makers. The adoption of actuarial methods across many industries was buoyed by concerns about racial equity following anti-discrimination legislation in the 1970s [@RN4794]. In a review of the litarture, @RN4794[p. 22] state that "evidence had accumulated that both private and public decision-makers were routinely giving into vague intuitions, personal prejudices, and arbitrary opinions. These decisions were deemed unfair in the sense that they were often skewed against low-status groups, or were simply inconsistent\ldots The mechanics of modern algorithms offered promises of transparency and of equal, dispassionate treatment—behind the veil of ignorance—without making distinctions based on prohibited demographic characteristics such as race or gender."

However, actuarial selection devices do not reduce racial inequality stemming from structural racism. @RN4814 criticizes social science disciplines (e.g., psychology, economics) for defining racism as an ideology held by individuals (e.g., explicit or implicit racial bias). These definitions cast attention to the attitudes and behaviors of individuals, ignoring the possibility that institutions can be racist. @RN4814 states that "racial ideology" -- commonly known as racism -- is the ideological component of racialized social systems, which allocate "economic, political, social, and even psychological rewards to groups along racial lines” (p. 474). This critique suggests that formulaic selection devices may produce racial inequality by incorporating [?seemingly neutral?] inputs that are correlated with race. @RN4778[p. xx] states that actuarial selection devices "tend to bake [racial] inequality into the decision-making process, transmuting social disadvantages into seemingly objective measures of individual riskiness." [SAY SOMETHING LIKE DISCRETIONARY JUDGMENT MAY ENABLE DECISION MAKERS TO OVERCOME STRUCTURALLY PROBLEMATIC INPUTS?] Interestingly, @RN4778 observes that the University of Michigan "point system" -- a standardized, but nonactuarial selection device -- effectively reduced racial inequality in college access by assigning points to underrepresented minority applicants, but this system was vulnerable to political and legal challenges.

We conceptualize student list products svis-a-vis scholarship on selection devices from sociology. First, student list products are quite literally selection devices in that they enable universities to select which prospective student profiles to purchase from a larger pool of prospective students. Second, student list products grant users great individual discretion in choosing which prospect profiles to purchase. For each purchase, a university may choose which inputs to filter on and which thresholds to apply to each filter. In contrast discretionary selection processes that rely on professional training and expertise (e.g., child welfare decisions, admissions readers), any individual associated with a Title IV institution may execute student list purchases, including consultants hired by the Title IV institution. Third, student list purchases are structured by which prospective students are in the underlying database, which search filters the product include, and which search filters the product encourages. Therefore, like actuarial selection devices, student list purchases may yield racial inequality due to search filters that are correlated with race. However, whereas actuarial selection devices replace individual judgment with decisions based on a formula, the individual discretion granted by student list products may reduce or amplify structural racial inequality embedded within student list products.


## Selection Device Inputs

In addition to how inputs are weighted, a fundamental aspect of selection devices is which inputs they incorporate in order to allocate individuals to categories. Choice of inputs relates to racial inequality in two broad ways. First, individuals with missing values for input variables are often excluded from the underlying database that the selection device pulls from, raising the possibility of selection bias. If missingness is correlated with race, then the selection device yields systematic racial exclusion. Guidance from the Federal Trade Commission states that "If a data set is missing information from particular populations, using that data to build an AI model may yield results that are unfair or inequitable to legally protected groups" [@RN4749].

Second, selection devices may produce racial inequality by utilizing inputs that are correlated with race. Drawing from Bonilla Silva's [-@RN4814] concept of "racialized social systems," structural racism is defined as “a form of systematic racial bias embedded in the ‘normal’ functions of laws and social relations” [@RN4760, p. 1143], whereby processes viewed as neutral or common-sense systematically advantage dominant groups and disadvantage marginalized groups. Following anti-discrimination legislation, many selection devices are "colorblind" in that they do not include race/ethnicity as an input, for example the child welfare selection device studied by @RN4778. Sociologists show that formally colorblind selection devices often reproduce racial inequality by utilizing seemingly neutral, objective, or universal determinants that are systematically correlated with race [@RN4775; @RN4786; @RN4849].

WHAT DAVON SAYS:

- Yeah, I think so. I argued in that piece that a racialized input is both empirically correlated with race and theoretically connected to histories of racial subjugation and exclusion to put some conceptual bounds on the term. 


@RN4786[p. 5] defines "racialized inputs" as "those that are theoretically and empirically correlated with historical racial disadvantage" versus non-racialized inputs defined as "those that are theoretically and emirically orthogonal or distant from racial disadvantage." Empirically, @RN4786 reconstructs Moody's city government credit rating algorithm, which assigns credits scores to cities based on determinants thought to predict loan default. @RN4786 conceptualizes median family income as a racialized input in that cities with a greater share of Black residents have lower median income because of historical wage discrimination. Median income is correlated with percent Black and once median income is included in the model, percent Black is no longer a significant predictor of city credit rating. Through the inclusion of seemingly neutral racialized inputs, "prior disadvantage and racism against Black individuals becomes institutionalized" [p. 2] and selection devices yield racially disparate outcomes "in ways that escape legibility/cognition as racially unequal" [p. 5].
...
PX: motivate hypotheses about race

- FICO score results in same likelihood to repay for same score across race. By contrast, sat less predictive of student success for black students.
