...
Proposition **P3** focuses on geographic filters. We create measures for student's school zip code by merging in data from the National Center for Education Statistics (NCES). Next, we attach income data to zip code by merging in data from the American Community Survey (ACS) 2012 5-year estimates. 
...
_Note: Filters used across these orders include HS Class, GPA (B-A+), State (in-state vs. out-of-state), AP STEM (3 min for in-state; 4 min for out-of-state) or SAT (1200 minimum for in-state; 1300 minimum for out-of-state) with STEM major interest_
\endgroup
...
\begingroup
\fontsize{10}{10}\selectfont
_Note: Filters used across these orders include HS Class, Segment, GPA (B-A+), PSAT/SAT (1220-1450); State/CBSAs_
\endgroup
